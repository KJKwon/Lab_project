library(ggplot2)
tbl = read.table('SH-SY5Y_IronTreat_2mMvs10mM_MGISeq.edgeRQLF_Robust_geneID_output.txt', sep = '\t', header = TRUE)
tbl.sig = read.table('SH-SY5Y_IronTreat_2mMvs10mM.MGISeq.edgeRQLF_Robust_geneID_output_clean.txt', sep = '\t', header = TRUE)
cut_FC = 1.00
cut_FDR = 0.05
...
p = ggplot(tbl_ready_ordered, aes(x = logFC, y = -log10(FDR), colour = group))+ geom_point(size = 1) +
  scale_color_manual(breaks = c('2mM treat up','10mM treat up'), values = c('#0066FF','#FF3300'),
                     guide = guide_legend(override.aes = list(color = c('#0066FF','#FF3300'))))+
