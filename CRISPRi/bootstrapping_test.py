...
def cor_output(sgRNA1, sgRNA2):
    ...
    total_sgRNA_clean = list(total_name)
    query1_clean = []
    query2_clean = []
    ...
    corr, _ = spearmanr(Query_1_clean, Query_2_clean) 
...
if len(sys.argv) < 2:
    ...
    f_out_nm = f_fasta1_nm.split('_S')[0] + '_' + f_fasta2_nm.split('_S') +'_correlation_bootstrap_output.txt'
    f_out = (f_out_nm,'w')
    f_ref = open('broadgpp-dolcetto-targets-set.txt','r')
    sgRNA2num = dict()
    for line in f_ref:
